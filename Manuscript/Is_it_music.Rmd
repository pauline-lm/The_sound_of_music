...
ratings_long <- pivot_longer(ratings, 
                         cols = c(monra, pulse, rhythm, repetition, melody, instrumental, timbre, tempo, intentionality, harmony, complexity), # dplyr::select columns to pivot
...
ratings_long2 <- pivot_longer(ratings, 
                         cols = c(pulse, rhythm, repetition, melody, instrumental, timbre, tempo, intentionality, harmony, complexity), # dplyr::select columns to pivot
                         names_to = "variable",     # new column name for variable names
                         values_to = "value") %>%
   mutate(levels = case_when(
    variable %in% c("instrumental", "intentionality", "complexity", "repetition") ~ 3,
...
We do the k-means clustering here. This is because the clusters will then be used for further analysis etc. and it is more convenient to do the kmeans clustering already in this section.
...
Now we add the clusters to different dataframes to be able to work on them later.
...
ratings_order <- ratings %>%
  mutate(order = case_when(
    ...
    label == "ambiguous" ~ 2,
    label == "non-music" ~ 3,
    TRUE ~ NA_real_  # in case there are other or missing values
  ))

ratings_order$order <- as.factor(ratings_order$order)
...
openness_mean <- mean(all_scales$total_openness, na.rm = TRUE)
openness_sd <- sd(all_scales$total_openness, na.rm = TRUE)
openness_min <- min(all_scales$total_openness, na.rm = TRUE)
openness_max <- max(all_scales$total_openness, na.rm = TRUE)

bais_mean <- mean(all_scales$bais, na.rm = TRUE)
bais_sd <- sd(all_scales$bais, na.rm = TRUE)
bais_min <- min(all_scales$bais, na.rm = TRUE)
bais_max <- max(all_scales$bais, na.rm = TRUE)

gold_mean <- mean(all_scales$gold, na.rm = TRUE)
gold_sd <- sd(all_scales$gold, na.rm = TRUE)
gold_min <- min(all_scales$gold, na.rm = TRUE)
gold_max <- max(all_scales$gold, na.rm = TRUE)

cat(
  paste0("The mean of total openness is ", round(openness_mean, 2), 
         ", with a standard deviation of ", round(openness_sd, 2), 
         ". The minimum value is ", round(openness_min, 2), 
         " and the maximum value is ", round(openness_max, 2), ".\n\n"),

  paste0("The mean of BAIS is ", round(bais_mean, 2), 
         ", with a standard deviation of ", round(bais_sd, 2), 
         ". The minimum value is ", round(bais_min, 2), 
         " and the maximum value is ", round(bais_max, 2), ".\n\n"),

  paste0("The mean of Gold-MSI is ", round(gold_mean, 2), 
         ", with a standard deviation of ", round(gold_sd, 2), 
         ". The minimum value is ", round(gold_min, 2), 
         " and the maximum value is ", round(gold_max, 2), ".")
)
...
ACOUSTIC FEATURE dplyr::selectION
...
numeric_mir <- acoustics_mir_scaled[, 4:251]
  
highly_corr<-fuzzySim::corSelect(numeric_mir, cor.thresh = .75, var.cols =1:248)
...
# Print only the first 10 features that have abvolsute highest correlations with music ratings (that we will pick)
print(head(full_cor_df, 10))

```


Now, dplyr::select the 10 acoustic features that have the abolsute highest correlation to mean_monra
```{r}
...
We reduce the dimensionality of our perceptual features to later use these principal components in modeling. We dplyr::select the first two principal components here. 
...
pca_data_perc$label <- acoustics_cluster_absolute_corr$label
...
PCA to reduce the number of acoustic features, we dplyr::select the first 2 principal components. 
...
Note: The signs of the principal components in this script may differ from those reported in the paper, as the component directions were flipped for easier interpretation (although they remained unchanged here). This transformation does not affect the validity or statistical results of the model.
...
Selection of the number of classes: 7 criteria (see https://www.displayr.com/work-number-classes-latent-class-analysis/#)
...
rand(class1_models)
performance::icc(class1_models, by_group = TRUE)
...
car::vif(class3_models) 
rand(class3_models)
performance::icc(class3_models, by_group = TRUE)
...
car::vif(class4_models) 
rand(class4_models)
performance::icc(class4_models, by_group = TRUE)
...
car::vif(class5_models) 
rand(class5_models)
performance::icc(class5_models, by_group = TRUE)
```
